...
\begin{figure}[tb]
  ...
  \begin{subfigure}[b]{1\linewidth}
    ...
    \end{itemize}
    \caption{Function schema for PLFS\\} \label{fig:plfs}
  \end{subfigure}
...
return o
      \end{minted}
      \caption{Code schema for SIRIUS\\} \label{fig:sirius}
  \end{subfigure}
...
void recurseBranch(TObjArray *o) {
  ...
  for(TBranch *b=i.Next(); i.Next()!=0; b=i.Next()) {
    ...
    recurseBranch(b->GetListOfBranches());
      ...
      \end{minted}
      \caption{Pointer and Code schemas for HEP} \label{fig:hep}
  \end{subfigure}
...
produces a list of files and directories.  For example, because PLFS
deterministically creates files and directories based on the number of clients,
...
is 3 directories and 6 files, with 2 files per directory.
...
SIRIUS use case constructs the namespace using Lua (for sandboxing purposes) and
complicated partitioning logic. Tintenfisch provides a code schema that gives
users the flexiblity to write programs that generate the namespace.
...
containing metadata and would read them to locate Baskets using the code schema.
