...
generators. Namespace schemas and generators help clients and servers establish
an understanding of the final file system metadata shape and size ({\it i.e.} a
a ``generation" contract) that eliminates metadata overheads.
...
be used for a namespace schema that is balanced or bounded (not mutually exlusive)
This approach reduces the size of the namespace (helps PLFS and ROOT use cases)
facilitates lazy file system metadata generation when the metadata is needed
(helps ROOT and SIRIUS use cases).  Tintenfisch relies on the user to design effective
