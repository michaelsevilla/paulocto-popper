...
scalability challenges for (1) namespaces with many requests and (2) namespaces
that are too large. Tintensfisch addresses these challenges by defining
namespace schemas and proposing namespace generators.
...
A namespace generator compacts the metadata and allows clients/servers to
generate file system metadata.  Namespace generators can only be used for
balanced or bounded namespace schemas (not mutually exlusive) and the approach
reduces RPC load (addresses challenges 1 and 2) and facilitates lazy file
system metadata generation when the metadata is needed (addresses challenge 3).
Tintenfisch relies on the user to design effective namespace generators that
leverage domain-specific knowledge to get the highest performance. This
programmable storage approach~\cite{sevilla:eurosys17-malacology} helps
...
namespace generators: formula, code, and pointers.
